...
- (BOOL) appendData:(NSData *)data { 
	...
	while( true ) {
		...
		if( waitingForCRLF ) {
			...
			offset = [self offsetTillNewlineSinceOffset:(int)offset inData:workingData];
			if( -1 == offset ) {
		...
		if( nil == currentHeader ) {
			...
			int headerEnd = [self findHeaderEnd:workingData fromOffset:(int)offset];
			if( -1 == headerEnd ) {
		...
		int contentEnd = [self findContentEnd:workingData fromOffset:(int)offset];
