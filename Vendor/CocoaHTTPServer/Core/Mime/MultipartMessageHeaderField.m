...
- (id) initWithData:(NSData *)data contentEncoding:(NSStringEncoding)encoding {
	...
	NSInteger separatorOffset = findChar(bytes, (int)length, ':');
	...
	separatorOffset = findChar(bytes, (int)length, ';');
	...
	if( ![self parseHeaderValueBytes:bytes length:(int)length encoding:encoding] ) {
